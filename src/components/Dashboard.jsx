  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-white tracking-tight">Welcome Back, John Moyo</h1>
          ...
          <div className="divide-y divide-slate-800/80">
            {recentActivity.map(item => (
              <div key={item.id} className="p-3.5 flex items-center justify-between">
                ...
                <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded border ${item.statusClass}`}>
                  {item.status}
                </span>
              </div>
            ))}
        ...
        <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-2.5">
          {quickActions.map(action => (
            <button
              ...
              className="bg-[#0b0f19] border border-slate-800/80 hover:border-emerald-500/40 rounded-xl p-4 flex flex-col items-center justify-center gap-3 text-center transition-all group"
            >
